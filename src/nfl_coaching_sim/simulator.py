...
class DeterministicSimulator:
    ...
    def fit(self, rows: Iterable[Mapping[str, Any]]) -> DeterministicSimulator:
        ...
        for row in rows:
            ...
            if (
                result is None
                or row.get("wpa") is None
                or row.get("epa") is None
                or not is_late_game_candidate(row)
            ):
                continue
            action, _ = result
        ...
        self.wpa_model = HistGradientBoostingRegressor(
            max_iter=150, max_depth=5, learning_rate=0.06, random_state=2026
        ).fit(x, np.asarray(wpa))
        self.epa_model = HistGradientBoostingRegressor(
            max_iter=150, max_depth=5, learning_rate=0.06, random_state=2026
        ).fit(x, np.asarray(epa))
    ...
    def candidates(self, scenario: Scenario) -> dict[Action, tuple[float, float, float]]:
        return {
            action: self._predict(scenario, action)
